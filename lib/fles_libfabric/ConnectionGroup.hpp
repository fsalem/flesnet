...
public:
    ...
    ConnectionGroup(std::string local_node_name)
    {
        ...
        int res = fi_eq_open(Provider::getInst()->get_fabric(), &eq_attr, &eq_,
                             nullptr);
        if (res) {
            L_(fatal) << "fi_eq_open failed: " << res << "="
                      << fi_strerror(-res);
...
#pragma GCC diagnostic pop
            ...
            if (count > 0) {
                ...
                default: {
                    L_(fatal)
                        << "unknown error in fi_eq_readerr: " << err_event.err
    ...
    int poll_completion()
    {
        const int ne_max = conn_.size() * 10;
...
protected:
                      ...
                      const std::vector<std::string>& compute_services)
    {
        ...
        if (Provider::getInst()->has_av()) {
            ...
            assert(res == 0);
            if (!av_) {
                L_(fatal) << "fi_av_open failed: " << res << "="
                          << fi_strerror(-res);
