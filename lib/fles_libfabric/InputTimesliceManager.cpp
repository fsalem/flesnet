...
void InputTimesliceManager::refill_future_timeslices(uint64_t up_to_timeslice)
{
    ...
    for (uint64_t ts = next_start_future_timeslice_; ts < up_to_timeslice;
         ++ts) {
        ...
        future_conn_timeslices_
            .get(virtual_physical_compute_mapping_[comp_index])
            ->insert(ts);
        comp_index = (comp_index + 1) % virtual_compute_count_;
    }
    ...
    uint32_t compute_index)
{
    ...
    while (it != to_be_moved_timeslices_.get_end_iterator()) {
        L_(debug) << "[" << compute_index << "]check to add: trigger "
                  << it->first << " next "
                  << get_connection_next_timeslice(compute_index) << " list "
                  << it->second[compute_index];
        if (get_connection_next_timeslice(compute_index) > it->first &&
            it->second[compute_index] != nullptr &&
            !it->second[compute_index]->empty()) {
            L_(debug)
                << "adding rescheduled timeslices after " << it->first << " to "
    ...
    uint64_t timeslice_trigger)
{
    ...
    for (uint32_t conn = 0; conn < compute_count_; ++conn) {
        ...
        std::set<uint64_t>* future_timeslices =
            future_conn_timeslices_.get(conn);
        while (!future_timeslices->empty()) {
            last_timeslice = *(--future_timeslices->end());
            if (last_timeslice <= timeslice_trigger) {
                L_(debug) << "last_timeslice of " << conn << " is "
                          << last_timeslice;
                break;
            }
...
InputTimesliceManager* InputTimesliceManager::get_instance(
    uint32_t scheduler_index, uint32_t compute_conn_count,
                                                        ...
                                                        uint64_t size)
{
    ...
    uint64_t descriptor_index = last_conn_desc_[compute_index] + 1,
             timeslice_data =
                 size + (last_conn_timeslice_[compute_index] ==
    ...
    assert(conn_timeslice_info_.get(compute_index)
               ->add(timeslice, timeslice_info));
    ...
    uint32_t compute_index, uint64_t timeslice)
{
    if (!conn_timeslice_info_.get(compute_index)->contains(timeslice)) {
        L_(warning) << "[i_" << scheduler_index_ << "][ACK_RDMA_WRITE] ts "
                    << timeslice << " does not belong to conn_"
                    << compute_index;
        return false;
    }
                                                    ...
                                                    uint64_t timeslice)
{
    ...
    TimesliceInfo* timeslice_info =
        conn_timeslice_info_.get(compute_index)->get(timeslice);
    return timeslice_info->rdma_acked_duration != 0 ||
                                                   ...
                                                   uint64_t descriptor)
{
    ...
    do {
        ...
        if (timeslice_info_it->second->compute_desc == descriptor)
            return timeslice_info_it->first;
...
InputTimesliceManager::get_last_rdma_acked_timeslice(uint32_t compute_index)
{
    ...
    do {
        ...
        if (timeslice_info->rdma_acked_duration != 0)
            return incompleted_it->second;

    } while (incompleted_it !=
             incompleted_desc_timeslices->get_begin_iterator());
    ...
    return get_timeslice_by_descriptor(compute_index,
                                       incompleted_it->first - 1);
}

uint64_t InputTimesliceManager::get_last_timeslice_before_blockage(
    uint32_t timed_out_conn)
{
    // TODO
    return 0;
}
...
{
    if (!conn_timeslice_info_.get(compute_index)->contains(timeslice)) {
        L_(fatal) << "compute index " << compute_index << " timeslice "
                  << timeslice << " last conn_timeslice "
    ...
    TimesliceInfo* timeslice_info =
        conn_timeslice_info_.get(compute_index)->get(timeslice);
    return std::pair<uint64_t, uint64_t>(timeslice_info->data,
                                         timeslice_info->compute_desc);
}
    ...
    const std::set<uint32_t> timeout_connections)
{
    ...
    for (uint32_t i = 0; i < compute_count_; ++i) {
        L_(debug) << "last transmitted ts of " << i << " is "
                  << last_conn_timeslice_[i] << " and desc "
                  << last_conn_desc_[i];
    ...
    const std::set<uint32_t> timeout_connections)
{
    ...
    std::set<uint64_t>* failed_timeslice =
        future_conn_timeslices_.get(failed_node_info.index);
    // Distribute failed_timeslices over other active connections in a temporary
    // array
    if (!failed_timeslice->empty()) {
        ...
        to_be_moved_timeslices_.add(failed_node_info.timeslice_trigger,
                                    movable_ts);
    }
}
...
void InputTimesliceManager::generate_log_files()
{
    ...
    block_log_file.open(log_directory_ + "/" +
                        std::to_string(scheduler_index_) +
                        ".input.ts_info.out");
    ...
    for (uint32_t i = 0; i < conn_timeslice_info_.size(); ++i) {
        ...
        while (delaying_time !=
               conn_timeslice_info_.get(i)->get_end_iterator()) {
            block_log_file << std::setw(25) << delaying_time->first
                           << std::setw(25) << i << std::setw(25)
