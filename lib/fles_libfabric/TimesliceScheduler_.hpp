...
public:
	...
	void init_input_index_info(uint32_t input_index,
		...
		L_(info) << "[" << compute_index_ << ":i" << input_index << "] offset " << sender_info_[input_index].clock_offset;
	}
...
*/
	...
	std::pair<std::chrono::system_clock::time_point, uint64_t> get_interval_sent_time(uint64_t interval_index){
		...
		if (slowdown_proposed_interval_+ (ConstVariables::SLOWDOWN_INTERVAL_PERIOD/2) >= interval_index &&
		    ...
		    slowdown_proposed_duration_+= (slowdown_proposed_duration_ * ConstVariables::SLOWDOWN_ROUND_FACTOR);
		}
		...
		//L_(info) << "[" << interval_index << "] interval_len = " << INTERVAL_LENGTH << ", factor = " << ConstVariables::SPEEDUP_ROUND_FACTOR << ", enahnced = " << enhanced_interval_duration << ", diff = " << ((enhanced_interval_duration/INTERVAL_LENGTH)*ConstVariables::SPEEDUP_ROUND_FACTOR);
		enhanced_interval_duration -= (enhanced_interval_duration*ConstVariables::SPEEDUP_ROUND_FACTOR);
		...
		enhancement_factor_log = -1;
		enhanced_interval_duration = speedup_proposed_duration_ + (speedup_proposed_duration_ * ConstVariables::SLOWDOWN_ROUND_FACTOR);
