...
public:
    ...
    static void initialize(uint32_t scheduler_index,
                           ...
                           uint32_t interval_length, std::string log_directory,
                           bool enable_logging);
    ...
    static void mark_timeslice_transmitted(uint32_t compute_index,
                                           uint64_t timeslice, uint64_t size);

    // Log the duration to receive the completion event of rdma write operation
    static bool mark_timeslice_rdma_write_acked(uint32_t compute_index,
                                                uint64_t timeslice);

    // Log the duration of a timeslice until receiving the ack
    static void mark_timeslices_acked(uint32_t compute_index,
                                      uint64_t up_to_descriptor_id);

    // Check whether a timeslice is acked
    static bool is_timeslice_rdma_acked(uint32_t compute_index,
                                        uint64_t timeslice);
    ...
    static uint64_t get_timeslice_by_descriptor(uint32_t compute_index,
                                                uint64_t descriptor);

    // Get last descriptor of a connection
    static uint64_t
    get_last_connection_descriptor_index(uint32_t compute_index);
    ...
    // timeslice
    static std::pair<uint64_t, uint64_t>
    get_data_and_desc_of_last_rdma_acked_timeslice(uint32_t compute_index);
    ...
    //
    static void log_timeslice_IB_blocked(uint32_t compute_index,
                                         uint64_t timeslice,
                                         bool sent_completed = false);

    //
    static void log_timeslice_CB_blocked(uint32_t compute_index,
                                         uint64_t timeslice,
                                         bool sent_completed = false);

    //
    static void log_timeslice_MR_blocked(uint32_t compute_index,
                                         uint64_t timeslice,
                                         bool sent_completed = false);
    ...
    static HeartbeatFailedNodeInfo* mark_connection_timed_out(
        uint32_t connection_id, uint64_t last_desc = ConstVariables::MINUS_ONE,
