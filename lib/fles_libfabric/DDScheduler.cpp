...
DDScheduler* DDScheduler::get_instance(
    uint32_t scheduler_index, uint32_t input_scheduler_count,
    uint32_t history_size, uint32_t interval_length,
    uint32_t speedup_difference_percentage, uint32_t speedup_percentage,
    uint32_t speedup_interval_count, std::string log_directory,
    bool enable_logging)
{
    if (instance_ == nullptr) {
        instance_ = new DDScheduler(
            scheduler_index, input_scheduler_count, history_size,
            interval_length, speedup_difference_percentage, speedup_percentage,
...
void DDScheduler::update_clock_offset(
    ...
    const uint64_t median_latency, const uint64_t interval_index)
{
...
void DDScheduler::generate_log_files()
{
    ...
    for (SizedMap<uint64_t, IntervalDataLog*>::iterator it =
        ...
        log_file << std::setw(25) << it->first << std::setw(25)
                 << it->second->min_start << std::setw(25)
                 << it->second->max_start << std::setw(25)
                 << it->second->min_duration << std::setw(25)
...
DDScheduler::DDScheduler(uint32_t scheduler_index,
                         uint32_t input_connection_count, uint32_t history_size,
...
void DDScheduler::calculate_interval_info(uint64_t interval_index)
{
    ...
    uint64_t average_last_timeslice =
        get_average_last_timeslice(interval_index);
    ...
    if (true) {
        L_(info) << "[" << scheduler_index_ << "] interval " << interval_index
                 ...
                 << average_last_timeslice << "] took "
                 << median_interval_duration << " us in " << average_round_count
                 << " rounds"
    ...
    if (interval_info_logger_.contains(interval_index)) {
        IntervalDataLog* interval_log =
            interval_info_logger_.get(interval_index);
        interval_log->min_start = min_start_time;
        interval_log->max_start = max_start_time;
        interval_log->min_duration =
            get_duration_statistics(interval_index, 0, 1);
        interval_log->max_duration =
...
DDScheduler::calculate_proposed_interval_meta_data(uint64_t interval_index)
{
    ...
    if (!proposed_interval_meta_data_.empty() &&
        ...
        new_start_timeslice =
            proposed_interval_meta_data_.get(interval_index - 1)
                ->last_timeslice +
            1;
    else
        new_start_timeslice =
    ...
    if (speedup_percentage_ > 0 && interval_dur_mean_difference >= 0 &&
        ...
        if (speedup_interval_index_ != 0 &&
            speedup_interval_index_ + speedup_interval_count_ + 1 ==
                interval_index) {
            last_stable_enhanced_interval_duration_ =
        ...
        if (true)
            L_(info) << "[" << scheduler_index_ << "] interval "
...
DDScheduler::get_start_time_statistics(uint64_t interval_index, bool average,
                                       bool min)
{
    ...
    for (uint32_t i = 1; i < input_connection_count_; i++) {
        tmp_time = input_scheduler_info_[i]
...
double DDScheduler::get_mean_round_duration_difference_distory()
{
    uint64_t last_completed_interval =
        actual_interval_meta_data_.get_last_key();
    ...
    for (uint32_t i = 0; i < history_size_; i++) {
        ...
        proposed_round_dur = proposed_interval->interval_duration /
                             proposed_interval->round_count;
...
void DDScheduler::update_compute_node_timeout_count(uint32_t timeout_count)
{
    ...
    if (compute_node_timeout_count_ != timeout_count) {
        ...
        speedup_interval_index_ =
            proposed_interval_meta_data_.get_last_key() +
