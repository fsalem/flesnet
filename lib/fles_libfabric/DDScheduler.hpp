...
{
/**
 * Singleton Distributed Deterministic Scheduler for compute nodes that could be used in TimesliceScheduler and ComputeNodeConnections!
 */
class DDScheduler
{
public:
...
private:
	...
	IntervalDataLog(uint64_t proposed_duration, uint64_t enhanced_duration, uint64_t rounds_count, bool speedup_applied):
		    min_start(0), max_start(0), min_duration(0), max_duration(0),
		    proposed_duration(proposed_duration), enhanced_duration(enhanced_duration), rounds_count(rounds_count), speedup_applied(speedup_applied){}

	// Constructor used when an entry is created after calculating the actual interval info
	IntervalDataLog(uint64_t min_start, uint64_t max_start, uint64_t min_duration,  uint64_t max_duration, uint64_t rounds_count):
	    min_start(min_start), max_start(max_start), min_duration(min_duration),
    ...
    // Calculate the statistics of an interval aftere receiving all the required actual meta-data
