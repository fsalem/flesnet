...
public:
    ...
    static DDScheduler*
    get_instance(uint32_t scheduler_index, uint32_t input_connection_count,
                 uint32_t history_size, uint32_t interval_duration,
...
private:
    ...
    struct IntervalDataLog {
        ...
        IntervalDataLog(uint64_t proposed_duration, uint64_t enhanced_duration,
                        uint64_t rounds_count, bool speedup_applied)
        ...
        IntervalDataLog(uint64_t min_start, uint64_t max_start,
    ...
    get_start_time_statistics(uint64_t interval_index, bool average = true,
                              bool min = false);
    ...
    uint64_t get_duration_statistics(uint64_t interval_index,
                                     bool average = true, bool min = false);
