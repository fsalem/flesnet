...
void InputChannelSender::report_status() {
	...
	SendBufferStatus status_desc { now, data_source_.desc_buffer().size(),
			cached_acked_desc_, acked_desc_, sent_desc_, written_desc };
	SendBufferStatus status_data { now, data_source_.data_buffer().size(),
			cached_acked_data_, acked_data_, sent_data_, written_data };
	...
	double rate_desc = static_cast<double>(status_desc.acked
			- previous_send_buffer_status_desc_.acked) / delta_t;
	double rate_data = static_cast<double>(status_data.acked
			- previous_send_buffer_status_data_.acked) / delta_t;
	...
	if (write_index_desc_ >= desc_offset + desc_length) {
		...
		uint64_t data_end =
				data_source_.desc_buffer().at(desc_offset + desc_length - 1).offset
						+ data_source_.desc_buffer().at(
								desc_offset + desc_length - 1).size;
		assert(data_end >= data_offset);

		uint64_t data_length = data_end - data_offset;
		uint64_t total_length = data_length
				+ desc_length * sizeof(fles::MicrosliceDescriptor);
...
std::unique_ptr<InputChannelConnection> InputChannelSender::create_input_node_connection(
		uint_fast16_t index) {
	// @todo
	...
	unsigned int max_pending_write_requests =
			std::min(static_cast<unsigned int>((max_send_wr - 1) / 3),
					static_cast<unsigned int>((num_cqe_ - 1)
							/ compute_hostnames_.size()));
...
void InputChannelSender::on_connected(struct fid_domain* pd) {
	if (!mr_data_) {
		...
		int err = fi_mr_reg(pd,
				const_cast<uint8_t*>(data_source_.data_send_buffer().ptr()),
		...
		err =
				fi_mr_reg(pd,
						const_cast<fles::MicrosliceDescriptor*>(data_source_.desc_send_buffer().ptr()),
...
void InputChannelSender::post_send_data(uint64_t timeslice, int cn,
		uint64_t desc_offset, uint64_t desc_length, uint64_t data_offset,
	...
	if ((desc_offset & data_source_.desc_send_buffer().size_mask())
		...
		// one chunk
		sge[num_sge].iov_base = &data_source_.desc_send_buffer().at(
				desc_offset);
		...
		sge[num_sge].iov_base = &data_source_.desc_send_buffer().at(
				desc_offset);
		sge[num_sge].iov_len = sizeof(fles::MicrosliceDescriptor)
	...
	if (data_length == 0) {
		...
		// one chunk
		sge[num_sge].iov_base = &data_source_.data_send_buffer().at(
				data_offset);
		...
		sge[num_sge].iov_base = &data_source_.data_send_buffer().at(
				data_offset);
		sge[num_sge].iov_len = data_source_.data_send_buffer().size()
...
void InputChannelSender::on_completion(uint64_t wr_id) {
	...
	case ID_WRITE_DESC: {
		...
		if (acked_data_ >= cached_acked_data_ + min_acked_data_
			...
			data_source_.set_read_index( { cached_acked_desc_,
					cached_acked_data_ });
		}
		if (false) {
