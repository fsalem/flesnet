...
void InputChannelSender::set_interval_proposed_info(InputIntervalInfo* interval_info){
    ...
    std::chrono::system_clock::time_point min_start_time;
	...
	std::pair<std::chrono::system_clock::time_point, uint64_t> proposed_info = conn_[i]->get_proposed_interval_info(interval_info->index);
    ...
    if (!found){
	min_start_time = std::chrono::system_clock::now();
...
InputIntervalInfo* InputChannelSender::create_interval_info(uint64_t interval_index){
    ...
    L_(info) << "New interval " << interval_index << ": starts " << interval_info->start_ts << ", ends " << interval_info->end_ts;
...
void InputChannelSender::check_send_timeslices()
{
	...
	interval_info->actual_start_time = std::chrono::system_clock::now();
    }
	...
	if (next_ts <= max_timeslice_number_ && interval_info->is_ts_within_current_round(next_ts)){
		...
		if (interval_info->cb_blocked){
		    ...
		    interval_info->cb_blocked_duration += std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now()-interval_info->cb_blocked_start_time).count();
		}
		if (interval_info->ib_blocked){
		    interval_info->ib_blocked = false;
		    interval_info->ib_blocked_duration += std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now()-interval_info->ib_blocked_start_time).count();
		}
		conn_[conn_index]->set_last_sent_timeslice(next_ts);
		...
		if (write_index_desc_ < desc_offset + desc_length) {
			...
			interval_info->ib_blocked_start_time = std::chrono::system_clock::now();
		    }
		    input_buffer_problem_count++;
		}else{
		    if (!interval_info->cb_blocked){
			interval_info->cb_blocked = true;
			interval_info->cb_blocked_start_time = std::chrono::system_clock::now();
		    }
	...
	ack_blocked_start_time_ = std::chrono::system_clock::now();
    }
	...
	if (ConstVariables::ENABLE_LOGGING){
		...
		uint64_t ack_blocked_time = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now() - ack_blocked_start_time_).count();
	    ...
	    int64_t scheduler_blocked_time = std::chrono::duration_cast<std::chrono::microseconds>(next_check_time - std::chrono::system_clock::now()).count();
	...
	next_check_time = std::chrono::system_clock::now() + std::chrono::microseconds(interval_info->get_duration_to_next_round());
    }
	...
	std::chrono::duration_cast<std::chrono::microseconds>(next_check_time - std::chrono::system_clock::now()).count(),
	...
	if (false){
	    L_(info) << "interval = " << interval_info->index << ", round = " << interval_info->rounds_counter <<
		    ...
		    ", sent = " << sent_count << " in " << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now() - now).count() <<
