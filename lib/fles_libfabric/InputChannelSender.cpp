      ...
      min_acked_data_(data_source.data_buffer().size() / 4)
{
    ...
    //// TODO TO BE REMOVED
    InputSchedulerOrchestrator::data_source_desc =
...
void InputChannelSender::send_timeslices()
{
    ...
    do {
        ...
        if (next_ts != ConstVariables::MINUS_ONE &&
            next_ts <= up_to_timeslice && next_ts <= max_timeslice_number_ &&
...
void InputChannelSender::operator()()
{
    try {
        ...
        while (InputSchedulerOrchestrator::sent_timeslices <=
                   max_timeslice_number_ &&
               !abort_) {
            /*uint32_t comp_indx =
            target_cn_index(InputSchedulerOrchestrator::sent_timeslices);
            uint64_t next =
            InputSchedulerOrchestrator::get_connection_next_timeslice(comp_indx);
        ...
        while (acked_desc_ <
               timeslice_size_ * InputSchedulerOrchestrator::sent_timeslices +
                   start_index_desc_) {
            poll_completion();
            scheduler_.timer();
        }
...
bool InputChannelSender::try_send_timeslice(uint64_t timeslice, uint32_t cn)
{
    ...
    if (write_index_desc_ >= desc_offset + desc_length) {
        ...
        if (true) {
            L_(debug) << "SENDER working on timeslice " << timeslice << " to "
                      << cn << ", microslices " << desc_offset << ".."
        ...
        if (conn_[cn]->check_for_buffer_space(total_length, 1)) {
            if (post_send_data(timeslice, cn, desc_offset, desc_length,
                               data_offset, data_length, skip)) {
                InputSchedulerOrchestrator::log_timeslice_CB_blocked(
                    cn, timeslice, true);

                // conn_[cn]->inc_write_pointers(total_length, 1);
                conn_[cn]->add_timeslice_data_address(total_length, 1);
                InputSchedulerOrchestrator::mark_timeslice_transmitted(
                    cn, timeslice, total_length);
                if (data_end >
                    sent_data_) { // This if condition is needed when the
...
// TODO TO BE REMOVED ... This is only used in the loop of sending timeslices in
// the operator() method
