...
ComputeHeartbeatManager::ComputeHeartbeatManager(uint32_t index,
                                                 ...
                                                 bool enable_logging)
    : HeartbeatManager(index, init_connection_count, log_directory,
                       enable_logging)
{
}
...
void ComputeHeartbeatManager::calculate_failure_decision(uint32_t failed_node)
{
    ...
    for (uint32_t i = 1; i < connection_count_; i++) {
        // select max desc
        L_(info) << "cur desc " << decision_info->last_completed_desc << " ["
                 << i << "] "
...
ComputeHeartbeatManager* ComputeHeartbeatManager::get_instance(
    uint32_t index, uint32_t init_connection_count, std::string log_directory,
    bool enable_logging)
{
    ...
    uint32_t connection_id, HeartbeatFailedNodeInfo failure_info)
{
    if (completed_decisions_log_.contains(failure_info.index)) {
        L_(warning) << "[" << index_ << "] heartbeat_failure from "
                    << connection_id
    ...
    if ((*collected_info)[connection_id]->failure_info != nullptr)
        return nullptr;
    (*collected_info)[connection_id]->failure_info =
        new HeartbeatFailedNodeInfo(failure_info.index,
...
{
    return completed_decisions_log_.contains(failed_connection_id) ? true
                                                                   : false;
}
                                               ...
                                               uint32_t failed_connection_id)
{
    ...
    if (completed_decision_acked_log_.find(failed_connection_id) !=
        ...
        L_(warning)
            << "[" << index_ << "] log_decision_ack from " << connection_id
    ...
    if (ack_list->find(connection_id) == ack_list->end()) {
        ...
        L_(info) << "[log_decision_ack]"
                 ...
                 << connection_id
                 << " acked the decision .. received count so far "
                 << ack_list->size();
    }
