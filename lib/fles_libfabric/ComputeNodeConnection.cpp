...
ComputeNodeConnection::ComputeNodeConnection(struct fid_eq* eq,
		...
		Connection(eq, connection_index, remote_connection_index), remote_info_(
				std::move(remote_info)), data_ptr_(data_ptr), data_buffer_size_exp_(
				data_buffer_size_exp), desc_ptr_(desc_ptr), desc_buffer_size_exp_(
...
ComputeNodeConnection::ComputeNodeConnection(struct fid_eq* eq,
		...
		fles::TimesliceComponentDescriptor* desc_ptr,
		uint32_t desc_buffer_size_exp) :
		Connection(eq, connection_index, remote_connection_index), data_ptr_(
				data_ptr), data_buffer_size_exp_(data_buffer_size_exp), desc_ptr_(
				desc_ptr), desc_buffer_size_exp_(desc_buffer_size_exp) {
...
void ComputeNodeConnection::setup_mr(struct fid_domain* pd) {
	assert(
			data_ptr_ && desc_ptr_ && data_buffer_size_exp_
...
void ComputeNodeConnection::inc_ack_pointers(uint64_t ack_pos) {
	...
	const fles::TimesliceComponentDescriptor& acked_ts = desc_ptr_[(ack_pos - 1)
