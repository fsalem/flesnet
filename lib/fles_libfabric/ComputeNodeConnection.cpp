...
void ComputeNodeConnection::on_complete_recv()
{
    ...
    if (!registered_input_MPI_time) {
    	...
    	timeslice_DD_scheduler_->init_input_scheduler(index_, recv_status_message_.local_time);
    	///-----
    ...
    if (recv_status_message_.actual_interval_metadata.interval_index != ConstVariables::MINUS_ONE) {
    	timeslice_DD_scheduler_->init_input_scheduler(index_, recv_status_message_.local_time, recv_status_message_.median_latency, recv_status_message_.actual_interval_metadata.interval_index);
