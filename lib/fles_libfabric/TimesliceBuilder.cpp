...
TimesliceBuilder::TimesliceBuilder(
    ...
    : ConnectionGroup(local_node_name), compute_index_(compute_index),
      ...
      ///-----
      ,
      timeslice_scheduler_(new TimesliceScheduler(
          compute_index_, num_input_nodes_,
          (uint16_t)ceil(ConstVariables::MAX_TIMESLICE_PER_INTERVAL /
                         num_input_nodes)))
///-----/
{
    ...
    timeslice_DD_scheduler_ = DDScheduler::get_instance(
        ...
        scheduler_interval_duration, scheduler_speedup_difference_percentage,
...
void TimesliceBuilder::build_time_file()
{
    ...
    if (true) {
        ...
        std::map<uint64_t, double>::iterator it =
            first_last_arrival_diff_.begin();
        ...
        if (true) {
            ...
            for (uint64_t desc = old_recv + 1; desc <= new_recv; desc++) {
                ...
                if (!first_arrival_time_.count(remote_ts_num)) {
                    first_arrival_time_.insert(
                        std::pair<
                            uint64_t,
                            std::chrono::high_resolution_clock::time_point>(
                            remote_ts_num,
                            std::chrono::high_resolution_clock::now()));
                    arrival_count_.insert(
                        std::pair<uint64_t, uint32_t>(remote_ts_num, 1));
                } else {
                    it = arrival_count_.find(remote_ts_num);
                    if ((it->second + 1) != conn_.size()) {
                        ++it->second;
                    } else {
                        first_last_arrival_diff_.insert(std::pair<uint64_t,
                                                                  double>(
                            remote_ts_num,
                            std::chrono::duration_cast<
                                std::chrono::microseconds>(
                                std::chrono::high_resolution_clock::now() -
                                first_arrival_time_.find(remote_ts_num)->second)
                                .count()));
...
void TimesliceBuilder::process_pending_complete_timeslices()
{
    ...
    for (uint64_t ts_pos : pending_complete_ts_) {
        ...
        if (!check_complete_timeslices(ts_pos))
            break;
        if (!drop_) {
            const fles::TimesliceComponentDescriptor& acked_ts =
    ...
    if (last_processed_ts != ConstVariables::MINUS_ONE) {
        std::set<uint64_t>::iterator last_processed_it =
            pending_complete_ts_.find(last_processed_ts);
        pending_complete_ts_.erase(pending_complete_ts_.begin(),
                                   ++last_processed_it);
    }

    // L_(info) << "End of process_pending_complete_timeslices";
    scheduler_.add(
        std::bind(&TimesliceBuilder::process_pending_complete_timeslices, this),
