    ...
    uint32_t compute_index, uint64_t up_to_descriptor_id)
{
    ...
    for (uint64_t desc = last_descriptor + 1; desc <= up_to_descriptor_id;
        ...
        if (timeslice == ConstVariables::MINUS_ONE) {
            L_(warning)
                << "Desc " << desc << " in compute conn_" << compute_index
    ...
    uint64_t avg_latency =
        timeslice_manager_->acknowledge_timeslices_completion(
            compute_index, up_to_descriptor_id);
...
bool InputSchedulerOrchestrator::is_timeslice_rdma_acked(uint32_t compute_index,
                                                         uint64_t timeslice)
{
    return timeslice_manager_->is_timeslice_rdma_acked(compute_index,
                                                       timeslice);
}
...
InputSchedulerOrchestrator::get_data_and_desc_of_last_timeslice(
    uint32_t compute_index)
{
    return timeslice_manager_->get_data_and_desc_of_last_timeslice(
        compute_index);
}
    ...
    HeartbeatFailedNodeInfo failed_node_info)
{
    ...
    L_(debug) << "Undo " << undo_timeslices.size()
              << " .... new sent_timeslices = " << sent_timeslices;
    // TODO
    InputSchedulerOrchestrator::last_timeslice_trigger =
        InputSchedulerOrchestrator::timeslice_trigger;
    ...
    uint32_t connection_id, uint64_t last_desc, uint64_t timeslice_trigger)
{
    ...
    return heartbeat_manager_->mark_connection_timed_out(
        connection_id, last_desc, timeslice_trigger);
}
...
InputSchedulerOrchestrator::get_timed_out_connection(int32_t timeout_conn)
{
    ...
    if (timeout_conn != -1) {
        HeartbeatFailedNodeInfo* failure_info = mark_connection_timed_out(
            ...
            get_up_to_timeslice_trigger(timeout_conn));
    ...
    uint32_t failed_compute_index)
{
    ...
    const std::set<uint32_t> timeout_connections =
        retrieve_timeout_connections();
    while (!timeout_connections.empty() &&
           timeslice_manager_->is_timeslice_belongs_to_timeout_connection(
               timeslice, timeout_connections)) {
        --timeslice;
    }
    ...
    uint64_t failed_compute_timeslices =
        timeslice_manager_->count_unacked_timeslices_of_interval(
            failed_compute_index, current_interval.start_ts,
            current_interval.end_ts) +
        timeslice_manager_->count_future_timeslices_of_interval(
            failed_compute_index, current_interval.start_ts,
            current_interval.end_ts);
