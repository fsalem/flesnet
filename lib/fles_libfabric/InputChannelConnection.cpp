...
InputChannelConnection::InputChannelConnection(struct fid_eq* eq,
		...
		Connection(eq, connection_index, remote_connection_index), max_pending_write_requests_(
...
void InputChannelConnection::send_data(struct iovec* sge, void** desc,
		int num_sge, uint64_t timeslice, uint64_t desc_length,
	...
	uint64_t cn_data_buffer_mask = (UINT64_C(1)
			<< remote_info_.data_buffer_size_exp) - 1;
	uint64_t cn_desc_buffer_mask = (UINT64_C(1)
			<< remote_info_.desc_buffer_size_exp) - 1;
	uint64_t target_bytes_left = (UINT64_C(1)
			<< remote_info_.data_buffer_size_exp)
...
#pragma GCC diagnostic ignored "-Wold-style-cast"
					sge2[num_sge2].iov_base = (uint8_t*) sge[i].iov_base
							+ target_bytes_left;
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wold-style-cast"
	send_wr_tscdesc.context = (void*) (ID_WRITE_DESC | (timeslice << 24)
			| (index_ << 8));
#pragma GCC diagnostic pop
...
void InputChannelConnection::connect(const std::string& hostname,
		...
		struct fid_cq* cq, struct fid_av* av, fi_addr_t fi_addr) {
