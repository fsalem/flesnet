...
void InputSchedulerOrchestrator::add_dfs_proposal(
    const DDSLoadBalancerMessage proposal, uint32_t compute_index) {
  if (proposal.proposed_interval_metadata.interval_index ==
      ConstVariables::MINUS_ONE)
    return;
  add_proposed_meta_data(proposal.proposed_interval_metadata,
                         proposal.last_completed_statistics, compute_index);
  timeslice_manager_->update_load_distribution(
      proposal.proposed_interval_metadata.start_timeslice,
      proposal.proposed_interval_metadata.last_timeslice,
      ConstVariables::array_to_vector(
          proposal.load_distribution,
          proposal.proposed_interval_metadata.compute_node_count));
}
void InputSchedulerOrchestrator::add_proposed_meta_data(
    const ComputeProposedIntervalMetaData meta_data,
    const ComputeIntervalMetaDataStatistics prev_interval_stats,
    uint32_t compute_index) {
  interval_scheduler_->add_proposed_meta_data(meta_data);
  interval_scheduler_->add_actual_interval_compute_statistics(
      prev_interval_stats.interval_index, compute_index, prev_interval_stats);
}
...
void InputSchedulerOrchestrator::retrieve_collected_interval_statistics(
  ...
  std::vector<ComputeIntervalMetaDataStatistics> compute_stats =
      interval_scheduler_->retrieve_actual_interval_compute_statistics(
          interval_index);
  for (uint32_t i = 0; i < compute_stats.size(); i++) {
    statistics[i] = compute_stats[i];
    if (false) {
      for (int c = 0; c < compute_stats.size(); c++)
        L_(info) << "[retrieve_collected_interval_statistics] interval: "
                 << interval_index << " i: " << c << " comp dur: "
...
uint64_t InputSchedulerOrchestrator::get_interval_index_of_timeslice(
    uint64_t timeslice) {
  return interval_scheduler_->get_interval_index_of_timeslice(timeslice);
}

uint64_t InputSchedulerOrchestrator::get_current_interval_index() {
  return interval_scheduler_->get_current_interval_info().index;
}

std::chrono::high_resolution_clock::time_point
InputSchedulerOrchestrator::get_expected_ts_sent_time(uint64_t timeslice) {
  uint64_t interval = get_interval_index_of_timeslice(timeslice);
  return interval_scheduler_->get_expected_ts_sent_time(interval, timeslice);
}
...
void InputSchedulerOrchestrator::mark_timeslice_transmitted(
  ...
  // TODO duplicated code between logger and TimesliceManager
...
bool InputSchedulerOrchestrator::mark_timeslice_rdma_write_acked(
  ...
  // TODO duplicated code between logger and TimesliceManager
  uint64_t interval = get_interval_index_of_timeslice(timeslice);
  return timeslice_manager_->acknowledge_timeslice_rdma_write(
...
void InputSchedulerOrchestrator::add_pending_dfs_message(
    std::uint32_t connection_id, IELoadBalancerMessage* message) {
  dfs_message_manager_->add_pending_message(connection_id, message);
}

IELoadBalancerMessage* InputSchedulerOrchestrator::get_pending_dfs_message(
    std::uint32_t connection_id) {
  return dfs_message_manager_->get_pending_IE_message(connection_id);
}
...
void InputSchedulerOrchestrator::log_timeslice_IB_blocked(
  ...
  timeslice_manager_->log_timeslice_IB_blocked(
      InputSchedulerOrchestrator::get_interval_index_of_timeslice(timeslice),
      compute_index, timeslice, sent_completed);
}

void InputSchedulerOrchestrator::log_timeslice_CB_blocked(
    uint32_t compute_index, uint64_t timeslice, bool sent_completed) {
  timeslice_manager_->log_timeslice_CB_blocked(
      InputSchedulerOrchestrator::get_interval_index_of_timeslice(timeslice),
      compute_index, timeslice, sent_completed);
}
...
void InputSchedulerOrchestrator::log_timeslice_MR_blocked(
    uint32_t compute_index, uint64_t timeslice, bool sent_completed) {
  timeslice_manager_->log_timeslice_MR_blocked(
      InputSchedulerOrchestrator::get_interval_index_of_timeslice(timeslice),
      compute_index, timeslice, sent_completed);
}
