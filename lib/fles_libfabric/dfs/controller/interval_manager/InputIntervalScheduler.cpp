...
bool InputIntervalScheduler::add_proposed_meta_data(
  ...
  proposed_interval_meta_data_.add(
      meta_data.interval_index, new ComputeProposedIntervalMetaData(meta_data));
  if (true) {
...
void InputIntervalScheduler::add_actual_interval_compute_statistics(
    uint64_t interval_index,
    uint32_t compute_index,
    ComputeIntervalMetaDataStatistics stats) {
  assert(actual_interval_compute_statistics_.contains(interval_index));
  std::vector<ComputeIntervalMetaDataStatistics>* compute_stats =
      actual_interval_compute_statistics_.get(interval_index);
  assert(compute_stats->size() > compute_index);
  compute_stats->at(compute_index) = stats;
  assert(actual_interval_compute_statistics_.get(interval_index)
             ->at(compute_index)
             .median_timeslice_completion_duration ==
         stats.median_timeslice_completion_duration);
  compute_stats = actual_interval_compute_statistics_.get(interval_index);
  if (true)
    L_(info)
        << "[add_actual_interval_compute_statistics] i " << interval_index
        << " c " << compute_index << " stats: buffer{"
        << ConstVariables::array_to_string(
               (*compute_stats)[compute_index].median_buffer_level,
               get_compute_connection_count())
        << "} comp_dur "
        << (*compute_stats)[compute_index].median_timeslice_completion_duration
        << " proc_dur "
        << (*compute_stats)[compute_index].median_timeslice_processing_duration;
}

const InputIntervalMetaData*
...
std::vector<ComputeIntervalMetaDataStatistics>
InputIntervalScheduler::retrieve_actual_interval_compute_statistics(
    uint64_t interval_index) {
  assert(actual_interval_compute_statistics_.contains(interval_index));
  return *actual_interval_compute_statistics_.get(interval_index);
}
...
InputIntervalScheduler::get_ack_sent_remaining_gap(uint64_t interval_index) {
  ...
  if (proposed_interval_meta_data_.contains(interval_index)) {
    ComputeProposedIntervalMetaData* next_interval =
...
InputIntervalScheduler::InputIntervalScheduler(uint32_t scheduler_index,
  ...
  minimum_ack_percentage_to_start_new_interval_ = 0.995;
}
...
void InputIntervalScheduler::create_new_interval_info(uint64_t interval_index) {
  ...
  if (proposed_interval_meta_data_.contains(
    ...
    ComputeProposedIntervalMetaData* proposed_meta_data =
        proposed_interval_meta_data_.get(interval_index);
...
void InputIntervalScheduler::create_actual_interval_meta_data(
  ...
  uint32_t compute_connection_count = get_compute_connection_count();
  InputLoggerProxy* logger = InputLoggerProxy::get_instance();
  std::vector<uint64_t> remote_blockage = logger->retrieve_sum_blockage(
                            InputLoggerProxy::LOG_TYPE::REMOTE_BLOCKAGE,
                            interval_info->index),
                        local_blockage = logger->retrieve_sum_blockage(
                            InputLoggerProxy::LOG_TYPE::LOCAL_BLOCKAGE,
                            interval_info->index);
  if (remote_blockage.size() < compute_connection_count)
    remote_blockage.resize(compute_connection_count, 0);
  if (local_blockage.size() < compute_connection_count)
    local_blockage.resize(compute_connection_count, 0);
  InputIntervalMetaData* actual_metadata = new InputIntervalMetaData(
      interval_info->index, interval_info->round_count, interval_info->start_ts,
      interval_info->end_ts, interval_info->actual_start_time,
      interval_info->actual_duration, compute_connection_count, remote_blockage,
      local_blockage,
      logger->retrieve_message_median_latency(interval_info->index),
      logger->retrieve_rdma_median_latency(interval_info->index));
  ...
  if (true) {
    L_(info) << "[i " << scheduler_index_ << "] "
             ...
             << interval_info->rounds_counter << " rounds, Local Blockage {"
             << ConstVariables::array_to_string(
                    actual_metadata->statistics.sum_IB_blockage_durations,
                    actual_metadata->compute_node_count)
             << "} Remote blockage {"
             << ConstVariables::array_to_string(
                    actual_metadata->statistics.sum_CB_blockage_durations,
                    actual_metadata->compute_node_count)
             << "} median message latency {"
             << ConstVariables::array_to_string(
                    actual_metadata->statistics.median_message_latency,
                    actual_metadata->compute_node_count)
             << "} median rdma latency {"
             << ConstVariables::array_to_string(
                    actual_metadata->statistics.median_rdma_latency,
                    actual_metadata->compute_node_count)
             << "}";
  }
  actual_interval_meta_data_.add(interval_info->index, actual_metadata);
  actual_interval_compute_statistics_.add(
      interval_info->index, new std::vector<ComputeIntervalMetaDataStatistics>(
                                actual_metadata->compute_node_count));
}
...
uint64_t
InputIntervalScheduler::get_interval_index_of_timeslice(uint64_t timeslice) {
  InputIntervalInfo* interval = get_interval_of_timeslice(timeslice);
  if (interval == nullptr)
    assert(false);
...
void InputIntervalScheduler::generate_log_files() {
  ...
  SizedMap<uint64_t, InputIntervalMetaData*>::iterator it_actual =
      actual_interval_meta_data_.get_begin_iterator();
  ComputeProposedIntervalMetaData* proposed_metadata = nullptr;
