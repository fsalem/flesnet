...
public:
  ...
  bool add_proposed_meta_data(const ComputeProposedIntervalMetaData);

  //
  void add_actual_interval_compute_statistics(
  ...
  const InputIntervalMetaData* get_actual_meta_data(uint64_t);

  //
  std::vector<ComputeIntervalMetaDataStatistics>
  retrieve_actual_interval_compute_statistics(uint64_t interval_index);
  ...
  uint64_t get_interval_index_of_timeslice(uint64_t);
...
private:
  ...
  SizedMap<uint64_t, InputIntervalMetaData*> actual_interval_meta_data_;

  // <interval, compute meta-data stats>
  SizedMap<uint64_t, std::vector<ComputeIntervalMetaDataStatistics>*>
      actual_interval_compute_statistics_;
