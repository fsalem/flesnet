...
void HeartbeatManager::ack_message_received(uint64_t messsage_id){
    if (!heartbeat_message_log_.contains(messsage_id))return;
    HeartbeatMessageInfo* message_info = heartbeat_message_log_.get(messsage_id);
    if (message_info->acked) return;
    message_info->acked = true;
    message_info->completion_time = std::chrono::high_resolution_clock::now();

    // remove pending unacked entries
    std::set<uint64_t>::iterator pending = unacked_sent_messages_.get(message_info->dest_connection)->find(messsage_id);
    if (pending != unacked_sent_messages_.get(message_info->dest_connection)->end()){
	unacked_sent_messages_.get(message_info->dest_connection)->erase(unacked_sent_messages_.get(message_info->dest_connection)->begin(), ++pending);
    }
}
