                         ...
                         struct fid_cq* cq, struct fid_av* av)
{
    ...
    // Provider::get_hints(Provider::getInst()->get_info()->ep_attr->type,
...
#pragma GCC diagnostic ignored "-Wold-style-cast"
    if (Provider::getInst()->has_eq_at_eps()) {
        ...
        if (err) {
            L_(fatal) << "fi_ep_bind failed: " << err << "="
                      << fi_strerror(-err);
...
#pragma GCC diagnostic ignored "-Wold-style-cast"
    err = fi_ep_bind(ep_, (::fid_t)eq_, 0);
    if (err) {
        L_(fatal) << "fi_ep_bind failed to eq: " << err << "="
                  << fi_strerror(-err);
    ...
    if (err) {
        L_(fatal) << "fi_ep_bind failed to cq: " << err << "="
                  << fi_strerror(-err);
...
void Connection::setup_heartbeat_mr(struct fid_domain* pd)
{
    ...
    res = fi_mr_reg(pd, &send_heartbeat_message_,
                    sizeof(send_heartbeat_message_), FI_SEND | FI_TAGGED, 0,
                    Provider::requested_key++, 0, &mr_heartbeat_send_, nullptr);
    if (res) {
...
void Connection::post_send_heartbeat_message()
{
    if (true) {
        L_(info) << "[i" << remote_index_ << "] "
                 ...
                 << send_heartbeat_message_.failure_info.index
                 << " failed desc "
